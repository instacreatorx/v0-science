...
export const authApi = {
  ...
  updateUser: (data: { name?: string; bio?: string; avatar?: string }) =>
...
export const articlesApi = {
  ...
  getTrendingArticles: (limit = 6) =>
    request<Article[]>(`/api/articles/trending?limit=${limit}`),
  
  searchArticles: (query: string, page = 1, perPage = 10) =>
    request<PaginatedResponse<Article>>(`/api/articles/search?q=${encodeURIComponent(query)}&page=${page}&per_page=${perPage}`),
  
  createArticle: (data: { title: string; excerpt: string; content: string; image?: string; tags?: string[]; is_member_only?: boolean }) =>
    request<Article>('/api/articles', { method: 'POST', body: data }),
  
  updateArticle: (id: number | string, data: { title?: string; excerpt?: string; content?: string; image?: string; tags?: string[]; is_member_only?: boolean }) =>
  ...
  bookmarkArticle: (id: number | string) =>
    request<{ bookmarked: boolean; message: string }>(`/api/articles/${id}/bookmark`, { method: 'POST' }),
};

// Comments API
export const commentsApi = {
  getComments: (articleId: number | string, page = 1, perPage = 20) =>
    request<PaginatedResponse<Comment>>(`/api/articles/${articleId}/comments?page=${page}&per_page=${perPage}`),
  ...
  deleteComment: (articleId: number | string, commentId: number | string) =>
    request<{ message: string }>(`/api/articles/${articleId}/comments/${commentId}`, { method: 'DELETE' }),
};
